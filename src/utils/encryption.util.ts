...
const SECRET_KEY = process.env.APP_SECRET 
  ? crypto.createHash('sha256').update(String(process.env.APP_SECRET)).digest('base64').substring(0, 32)
...
export class EncryptionUtil {
  ...
  static decrypt(encryptedData: string): string {
    ...
    try {
      ...
      console.error('Dekripsi gagal. Data mungkin rusak atau kunci berubah.', error);
